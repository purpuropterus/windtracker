...
export const useHistoryEditorStore = defineStore("historyEditor", {
    ...
    actions: {
        ...
        historyRunthrough(){
            ...
            this.directionsToCheck = windStore.history.slice(Math.floor(windStore.historyLength / 8), -(settingsStore.holes - directionsNum)).map(group => group[0]).filter(id => id !== "?")
        

            //only check the last group of 9
            const speedsNum = windStore.historyLength % 9
            this.speedsToCheck = windStore.history.slice(Math.floor(windStore.historyLength / 9), -(settingsStore.holes - speedsNum)).map(group => group[1])
